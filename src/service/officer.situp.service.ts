...
export class OfficerSitupService{
    ...
    async createsituprecordbyother(createDto:CreateSitupByother,accountId:number,user:any):Promise<{message:string}>{
        ...
        const createRecord = await this.officersitupTest.create({
        ...
        await this.officersitupTest.save(createRecord)
        return{
            message:'Created Record succcessfully'
        }
    }
